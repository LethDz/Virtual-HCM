...
class Terminal extends Component {
  ...
  checkVersion = (comp, ver) => {
    ...
    if (!/^(\d|\.)+$/.test(ver)) {
      ...
      const checkBools = split.map((val, index) =>
        compLogic(comp)(realSplit[index], val)
      );
  ...
  registerInstance = (index, instance) => {
    ...
    pluginMap(this.props.plugins, (PluginClass, config) => {
      try {
        ...
        console.error(
          `Error instantiating plugin ${PluginClass.displayName}`,
          e
  ...
  checkShortcuts = (instance, key, e) => {
    ...
    if (controller !== null) {
      ...
      const instanceData = this.state.instances.find((i) =>
        isEqual(i.instance, instance)
      );
  ...
  handleChange = (instance, e) => {
    ...
    const saveToHistory =
      controller !== null ? controller.history || false : true;
  ...
  runCommandOnActive = (inputText, force = false) => {
    const data = this.state.instances.find(
      (i) => i.index === this.state.activeTab
    );
  ...
  printToActive = (...args) => {
    ...
    if (
      data &&
      data.instance !== null &&
      data.instance.state.controller === null
    ) {
